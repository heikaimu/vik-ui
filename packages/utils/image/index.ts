...
function imageReset(option: ImageResetParams): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    // 如果是base64
    if (typeof file === 'string' && (file as any).includes('data:image')) {
 ...
 */
function flipImage(data: Blob | string, type = 'base64', isJPG: boolean): Promise<string | Blob> {
  return new Promise((resolve, reject) => {
    ...
    image.onload = function () {
      ...
      const base64 = canvas.toDataURL(extention, 0.92)
